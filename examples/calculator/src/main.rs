...
//!
//! Then connect with any MCP client (e.g. Claude Desktop) via stdio.
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("calculator=info".parse()?)
                .add_directive("mcp=debug".parse()?),
        )
        .init();

    // ── Build the schema for BinaryInput using schemars ───────────────────────
    ...
    McpServer::builder()
        ...
        // ── sqrt: demonstrates Result return type ─────────────────────────────
        .tool(
        ...
        .tool(
            Tool::new(
                ...
                serde_json::json!({
                    "type": "object",
                    "properties": {
                        "n": { "type": "integer", "description": "Non-negative integer (≤ 20)", "minimum": 0, "maximum": 20 }
                    },
                    "required": ["n"]
                }),
            ),
