...
use crate::{
    handler::{PromptHandler, ResourceHandler, ToolHandler},
    router::Router,
    server::McpServer,
};
...
///     .name("my-server")
///     .version("1.0.0")
///     .instructions("I am a helpful server")
///     .tool(
///         Tool::new("add", "Add two numbers", schemars::schema_for!(AddParams).into()),
///         |params: AddParams| async move { CallToolResult::text(format!("{}", params.a + params.b)) },
///     )
///     .build();
/// ```
...
impl McpServerBuilder {
    ...
    ///
    /// The handler can be:
    /// - `|args: serde_json::Value| async { ... }` for raw JSON
    /// - `|params: MyStruct| async { ... }` for typed (auto-deserialised) params
    ...
    /// Convenience: register a simple no-parameter tool.
    pub fn tool_fn<M>(
        mut self,
        ...
        let tool = Tool::no_params(name, description);
        self.router.add_tool(tool, handler.into_handler_fn());
        self
    }
...
pub struct ToolDef {
    pub tool: Tool,
    pub handler: crate::handler::ToolHandlerFn,
}

impl ToolDef {
    pub fn new(tool: Tool, handler: crate::handler::ToolHandlerFn) -> Self {
        Self { tool, handler }
    }
}
