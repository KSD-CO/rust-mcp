...
use mcp_core::{
    ...
    protocol::{JsonRpcError, JsonRpcMessage, JsonRpcNotification, JsonRpcRequest, JsonRpcResponse},
    types::{
        messages::{
...
///
/// Create one with `McpServer::builder()` and then call `.serve_stdio()` or
/// another transport method to start accepting connections.
#[derive(Clone)]
...
impl McpServer {
    ...
    async fn handle_initialize(
        ...
        let capabilities = ServerCapabilities {
            ...
            logging: Some(mcp_core::types::LoggingCapability {}),
            experimental: None,
        };

        let result = InitializeResult {
            protocol_version: mcp_core::protocol::MCP_PROTOCOL_VERSION.to_owned(),
            capabilities,
