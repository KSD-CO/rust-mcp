...
///   2. `|params: MyStruct| async { ... }` – typed, deserialized args (via Json<T>)
...
/// `Value` implements `DeserializeOwned` and deserializes from itself.
pub struct TypedMarker<T>(std::marker::PhantomData<T>);

// Typed: Fn(T) -> impl Future<Output = impl IntoToolResult>
