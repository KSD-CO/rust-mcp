...
///
/// Each GET /sse creates a new session. The client's session ID is sent as the
/// first SSE event. Subsequent POST /message calls must include ?sessionId=...
...
impl SseTransport {
    ...
    /// Start the HTTP/SSE server on the configured address.
    pub async fn serve(self) -> McpResult<()> {
        ...
        let listener = tokio::net::TcpListener::bind(self.addr)
            .await
            .map_err(mcp_core::error::McpError::Io)?;

        axum::serve(listener, app)
            .await
            .map_err(|e| mcp_core::error::McpError::Transport(e.to_string()))?;
...
async fn sse_handler(AxumState(state): AxumState<SseState>) -> Response {
    ...
    // First event: the endpoint URI the client should POST to
